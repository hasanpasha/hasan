...
def CheckLogin():
    with open(creds) as f:
        ...
        uname = data[1].rstrip() # Data[0], 0 is the first line, 1 is the second and so on.
        pword = data[3].rstrip() # Using .rstrip() will remove the \n (new line) word from before when we input it
    ...
    if nameEL.get() == uname and pwordEL.get() == pword: # Checks to see if you entered the correct data.
        ...
        rlbl = Label(r, text='\n[+] Logged In').grid(row=1, column=1) # "logged in" label
        with open(file) as h:
            data = h.readlines()
            text = data[0,1,2,3,4,5,6,7,8].rstrip()
            rlb2 = Label(r, text=text).grid(row=2, column=1)
        ...
        r.title('D:')
        r.geometry('150x50')
        rlbl = Label(r, text='\n[!] Invalid Login').grid(row=1, column=1)
        r.mainloop()
